...
pkg_setup() {
	...
	if use aac && ! built_with_use media-libs/faad2 mp4; then
		ewarn "mp4 support is disabled."
		ewarn "It's optional. but if you want to play *.mp4 file,"
		ewarn "You should use 3rd party ebuild from below url."
		ewarn "URL : http://nidev.tistory.com/attachment/4930d3ae73ff299.tbz2"
	fi
...
src_unpack() {
	unpack ${A}
	cd "${S}"
	epatch "${FILESDIR}"/mpdconf-beta2.patch
}
