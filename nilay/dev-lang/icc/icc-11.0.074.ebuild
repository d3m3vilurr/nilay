...
pkg_setup() {
	...
	src_unpack() {
		...
		if use x86; then
			cd "${S}"/${INSTALL_DIR}/bin/ia32/
		fi
		if use amd64; then
			cd "${S}"/${INSTALL_DIR}/bin/intel64/
		fi
		...
		chmod 644 *support
		# what a great trick!
		einfo "editting intel's stupid path."
		if use x86; then
			cd "${S}"/${INSTALL_DIR}/lib/ia32/
		fi
		if use amd64; then
			cd "${S}"/${INSTALL_DIR}/lib/intel64/
		fi
		# this may break...
		#sed -i 's|/usr/local/share/macrovision/storage|/tmp/|g' *libFNP* || die "sed support failed."
		# enhanced.
		python ${FILESDIR}/binpatcher.py *libFNP*
	}
